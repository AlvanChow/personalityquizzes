...
The app is iPhone-only (`TARGETED_DEVICE_FAMILY = 1`), so App Store Connect will
not ask for the 13" iPad set. Capture on the **largest iPhone simulator
available** — App Store Connect reuses that set for smaller devices. Confirm the
exact required size in the media manager, since Apple moves it.
