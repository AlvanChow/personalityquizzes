...
- **Screenshots** — 6.9"/6.7" iPhone only. The app is iPhone-only
  (`TARGETED_DEVICE_FAMILY = 1`), so App Store Connect does not ask for the 13"
  iPad set. Re-enabling iPad means producing those *and* auditing a mobile-first
  layout on a 13" canvas — see the Guideline 4.2 note below before doing it.
...
- **iPad**: off. Set `TARGETED_DEVICE_FAMILY = "1,2"` in both build
  configurations to turn it back on, and update the guard in
  `capacitorConfig.test.js`. `UISupportedInterfaceOrientations~ipad` already
  lists all four orientations, which is what keeps that from failing validation
  with ITMS-90474.
